  ...
  class Common
    ...
    class << self
      ...
      def to_normalized_string(text)
        ...
        col_text = text.to_s.gsub(Regexp.new(('(<[^>]*>)|\n|\t').encode('UTF-8')), ' ') # Remove html tags
        col_text.downcase!                            # Remove capitalization
        col_text.gsub!(Regexp.new(('/\"|\'/').encode('UTF-8')), '')                   # Remove potential problem characters
        col_text.gsub!(Regexp.new(('/\(.*?\)/').encode('UTF-8')),'')                  # Remove text inside parens
        col_text.gsub!(Regexp.new(('\W').encode('UTF-8')), ' ')                     # Remove all other non-word characters
